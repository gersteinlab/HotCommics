...
if (len(sys.argv) != 7):
   ...
   sys.stderr.write("   <binary_contact_map> is name of the output file containing the matrix of 1's and 0's for the contact map \n")
...
binary_contact_map = sys.argv[5]
binary_contact_map_print = open(binary_contact_map, "w")
ca_file = sys.argv[6]
...
while r < len(res_data):
	...
	binary_contact_map_print.write(str(res_data[r][3]) + " "  + str(res_data[r][4]) + " " + str(res_data[r][5]) + " " + str(res_data[r][6]) + "      ",)
	j = 0
	while j < len(res_data):
		binary_contact_map_print.write(str(contact_status[r][j]) + " ",)
		j += 1
	binary_contact_map_print.write("\n")
	r += 1
...
binary_contact_map_print.close()
